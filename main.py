...
async def manager(db: GameDB):
    ...
    if '1' in choice:
        ...
        old_year = is_digit_or_none(
            input(f'Введите старый год издания игры:\n'))
        ...
        new_year = is_digit_or_none(
            input(f'Введите новый год издания игры:\n'))

        os.system('cls||clear')

        if not any((new_title, new_publisher, new_year)):
            print(
                'Вы не ввели данных необходимых для обновления текущих\nДанные игры остались прежними')
            return
        ...
        if not games:
            os.system('cls||clear')
            print(
                "База данных пустая. Вам нужно добавить игры,\nчто бы пользоваться этой функцией")
            return
